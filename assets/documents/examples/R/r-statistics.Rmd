...
plot(gg)
canvasXpress(gg, width = 700, height = 500)
#ggplotly(gg)
```
...
theme_set(theme_bw())  # pre-set the bw theme.
g <- ggplot(mpg, aes(cty, hwy))
gg <- g + geom_count(col="tomato3", show.legend=F) +
  labs(subtitle="mpg: city vs highway mileage", 
       y="hwy", 
       x="cty", 
       title="Counts Plot")
...
gg <- g + geom_point(aes(col=manufacturer, size=hwy)) + 
...
# Diverging Barcharts
gg <- ggplot(mtcars, aes(x=`car name`, y=mpg_z, label=mpg_z)) + 
...
# Draw plot
gg <- ggplot(cty_mpg, aes(x=make, y=mileage)) + 
...
# Minify theme
gg <- p + theme(panel.background = element_blank(), 
...
gg <- g + geom_bar(aes(fill=class), width = 0.5) + 
  theme(axis.text.x = element_text(angle=65, vjust=0.6)) + 
...
gg <- g + geom_density(aes(fill=factor(cyl)), alpha=0.8) + 
...
gg <- g + geom_boxplot(varwidth=T, fill="plum") + 
    labs(title="Box plot", 
...
library(ggthemes)
g <- ggplot(mpg, aes(class, cty))
gg <- g + geom_boxplot(aes(fill=factor(cyl))) + 
  theme(axis.text.x = element_text(angle=65, vjust=0.6)) + 
  labs(title="Box plot", 
       subtitle="City Mileage grouped by Class of vehicle",
       caption="Source: mpg",
...
# Plot
gg <- ggplot(email_campaign_funnel, aes(x = Stage, y = Users, fill = Gender)) +   # Fill column
...
#pie <- ggplot(df, aes(x = "foo", y=freq, fill = factor(class))) + 
...
# plot
gg <- ggplot(economics_m, aes(x=date)) + 
  geom_line(aes(y=returns_perc)) + 
...
# plot
gg <- ggplot(economics_y, aes(x=date)) + 
  geom_line(aes(y=returns_perc)) + 
